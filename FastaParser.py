...
class FastaParser:
    def __init__(self, filepath):
        ...
        # custom regex for matching cleavage sites with KED/EDR patterns between them
        self.prohormone_regex = rf"(?:(?<=.{{{constants.SIGNAL_PEPTIDE_LENGTH}}})(?:(?<!K|R)(?:KK|KR|RR|RK)(?=[^RKILPVH]|(?<=KR)H|$))(?=.*?(?:KED|EDR).*?(?:(?<!K|R)(?:KK|KR|RR|RK)(?=[^RKILPVH]|(?<=KR)H|$))))"
        ...
        try:
            with open(filepath, "r") as fasta_file:
                ...
                self.sequences = [
                    (sequence, self.get_cleavage_sites(sequence))
                    for sequence in tqdm(fastaparser.Reader(fasta_file))
                ]
                self.set_matches()
    ...
    def cleavage_spacing(self, spacing):
        ...
        """
        self._cleavage_spacing = (
            constants.MIN_CLEAVAGE_SPACING
            if spacing < constants.MIN_CLEAVAGE_SPACING
            else spacing
        )

        # recompile regex with KED/EDR pattern requirement
        self.prohormone_regex = rf"(?:(?<=.{{{constants.SIGNAL_PEPTIDE_LENGTH}}})(?:(?<!K|R)(?:KK|KR|RR|RK)(?=[^RKILPVH]|(?<=KR)H|$))(?=.*?(?:KED|EDR).*?(?:(?<!K|R)(?:KK|KR|RR|RK)(?=[^RKILPVH]|(?<=KR)H|$))))"
        if self.sequences:
            print("Updating matches...")
            self.sequences = [
                (sequence, self.get_cleavage_sites(sequence))
                for sequence in tqdm([seq for seq, _ in self.sequences])
            ]
        self.set_matches()
    ...
    def num_cleavage_sites(self, num_cleavage_sites):
        ...
        """
        self._num_cleavage_sites = (
            constants.MIN_NUM_CLEAVAGE_SITES
            if num_cleavage_sites < constants.MIN_NUM_CLEAVAGE_SITES
            else num_cleavage_sites
        )
    ...
    def get_cleavage_sites(self, sequence):
        ...
        """
        return list(
            regex.finditer(self.prohormone_regex, sequence.sequence_as_string())
        )
    ...
    def set_matches(self):
        ...
        if self.sequences:
            print(
                f"Finding matches with at least {self.num_cleavage_sites} cleavage sites separated by at least {self.cleavage_spacing} amino acids..."
            )
    ...
    def graph_cleavage_density(self):
        ...
        """
        x, y = zip(
            *[(len(sequence), len(match)) for sequence, match in tqdm(self.matches)]
        )
        plt.scatter(x, y)
    ...
    def tissue_specificity(self, show_graph=False):
        ...
        urls = [
            f"http://www.proteinatlas.org/api/search_download.php?search={uniprot_from_fasta(sequence)}&format=json&columns=g,up,rnats,rnatsm&compress=no"
            for sequence, _ in self.sequences
        ]
        ...
        if show_graph:
            ...
            tissues, frequencies = zip(
                *sorted(tissue_expression.items(), key=lambda x: -x[1])
            )
    ...
    def extract_gene_name(self, header):
        """
        Extracts the gene name from a FASTA header.

        Parameters:
          header (str): The FASTA header string.

        Returns:
          str: The gene name or "Unknown" if not found.
        """
        import re
    ...
    def find_ked_edr_patterns(self, sequence):
        """
        Searches for KED or EDR patterns between cleavage sites in a sequence.

        Parameters:
          sequence (str): The sequence to be searched.

        Returns:
          list: Locations of KED or EDR patterns between cleavage sites.
        """
        ked_edr_regex = r"(?:KED|EDR|AED|AEDG|RGD)"

        # Get cleavage sites first
        cleavage_sites = self.get_cleavage_sites(sequence)

        if len(cleavage_sites) < 2:
            return []

        patterns = []
        # Search between each pair of consecutive cleavage sites
        for i in range(len(cleavage_sites) - 1):
            start = cleavage_sites[i].span()[1]
            end = cleavage_sites[i + 1].span()[0]

            # Search for KED/EDR in the region between cleavage sites
            segment = sequence.sequence_as_string()[start:end]
            matches = list(regex.finditer(ked_edr_regex, segment))

            # Adjust match positions to be relative to full sequence
            for match in matches:
                adjusted_span = (match.span()[0] + start, match.span()[1] + start)
                patterns.append((adjusted_span, match.group()))

        return patterns

    def check_all_patterns_in_sequence(self, sequence):
        """
        Checks for all KED/EDR/AED/AEDG/RGD patterns in the entire sequence,
        not just between cleavage sites, for debugging purposes.

        Parameters:
          sequence: The sequence to be searched.

        Returns:
          dict: Count of each pattern type found in the sequence.
        """
        pattern_counts = {"KED": 0, "EDR": 0, "AED": 0, "AEDG": 0, "RGD": 0}
        seq_string = sequence.sequence_as_string()

        for pattern in pattern_counts.keys():
            pattern_counts[pattern] = len(list(regex.finditer(pattern, seq_string)))
